...
def render(data):
    days = [d for d in data["days"] if "2025-12-01" <= d["date"] <= "2026-07-18"]
    if not days:
        days = data["days"]
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">199adarsh@github: ~/contributions --graph</text>')
    ...
    # compute current & longest streak for filtered range
    run = longest = cur = 0
    for d in days:
        if d["count"] > 0:
            run += 1
    ...
    parts.append(f'<text x="{PAD}" y="{ly}" font-size="13" fill="{GREEN}">'
                 ...
                 f'<tspan fill="{MUTED}"> contributions (Dec 2025 &#8594; Jul 2026)</tspan></text>')
